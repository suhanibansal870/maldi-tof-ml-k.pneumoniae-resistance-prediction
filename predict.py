...
st.title("K.pneumoniae-ertapenem-susceptibility")
st.subheader("Predict susceptibility or resistance from JSON features")
st.write(""" Klebsiella pneumoniae is a type of Gram-negative, non-motile, rod-shaped bacterium that is part of the Enterobacteriaceae family. It is commonly found in the environment, including in soil, water, and plants, and can also be part of the normal flora in the human intestines. While it is harmless in the gut, it can cause a range of infections if it spreads to other parts of the body.Klebsiella pneumoniae is best known for causing pneumonia, particularly in hospital settings, where it is a significant cause of hospital-acquired infections. It can lead to symptoms such as fever, cough, chest pain, and difficulty breathing. """)
st.header("Upload JSON data")
uploaded_file=st.file_uploader("Upload your spectral data (JSON only)",type=["json"],accept_multiple_files=False,help="Upload JSON file containing spectral data")
with st.sidebar:
        ...
        try:
            ...
            if st.button("RUN PREDICTION", type="primary"):
                with st.spinner("Analyzing Spectral data with all models..."):
                    ...
                    # Add Random Forest results (using "RF" from your image if needed)
                    results_data.append({
                        'Sample ID': sample_id,
                        'Model': 'RF',  # Using RF as shown in image
